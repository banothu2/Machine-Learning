			...
			for k in range(length(j)):
				...
				for z in range(num_elements_in_class):
					if seperatedByLabels[i][z][j] == k:
						count = count + 1
				#_attribute[k] = count # need to change this to count + 1 / total count + j
...
def test_accuracies(test_translated, MLE_classes, seperatedByLabels, training_translated):
	...
	#predicted_correctly_zero = 0
	#predicted_correctly_one = 0
	...
	#print "Accuracy = ", (((predicted_correctly_zero + predicted_correctly_one)*1.0)/ len(test_translated))
	return (((predicted_correctly[0] + predicted_correctly[1])*1.0)/ len(test_translated))
